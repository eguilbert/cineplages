<template>
  <div class="p-6 max-w-6xl mx-auto">
    <div class="mb-4">
      ...
      <div class="flex flex-wrap gap-4 text-sm mt-2">
        <div class="flex items-center gap-2"><span class="w-4 h-4 rounded bg-green-50 border border-green-200"></span> Enfants / scolaire (14:00)</div>
        <div class="flex items-center gap-2"><span class="w-4 h-4 rounded bg-yellow-50 border border-yellow-300"></span> Documentaire (16:00)</div>
        <div class="flex items-center gap-2"><span class="w-4 h-4 rounded bg-blue-50 border border-blue-200"></span> Art & essai (18:00)</div>
        <div class="flex items-center gap-2"><span class="w-4 h-4 rounded bg-red-50 border border-red-200"></span> Grand public (20:00)</div>
        <div class="flex items-center gap-2"><span class="w-4 h-4 rounded bg-purple-50 border border-purple-300"></span> Thriller / adulte (22:00)</div>
      </div>
    </div>
    <h1 class="text-2xl font-bold mb-4">🎬 Suggestions + Programmation Multi-Salles</h1>
    ...
    <div class="mb-6">
      ...
      <div class="flex items-center gap-4">
        <div>
          ...
          <input type="date" v-model="startDateInput" class="border p-2 rounded">
        </div>
        <div>
          <span class="text-sm text-gray-500">au</span>
          <input type="date" v-model="endDateInput" class="border p-2 rounded">
        </div>
      </div>
    </div>

    <div class="md:flex gap-6">
  <aside class="md:w-1/3 sticky top-20 self-start max-h-[80vh] overflow-y-auto pr-2 bg-gray-50 border border-gray-200 rounded-lg p-4 shadow-sm">
    <h2 class="text-xl font-semibold mb-4 text-gray-700">🎞️ Films disponibles</h2>
    ...
    <div v-if="filteredMovies.length" class="grid grid-cols-1 gap-4 mb-10">
      <div
        ...
        :key="movie.id"
        draggable="true"
        @dragstart="(e) => e.dataTransfer.setData('film', JSON.stringify(movie))"
        :class="[getSuggestedColorClass(movie.genres), 'p-4 border rounded-lg shadow hover:shadow-md transition cursor-move']"
      >
        <div class="font-semibold text-base text-gray-800 flex items-center justify-between">
  {{ movie.title }}
  <span class="ml-2 px-2 py-0.5 text-xs rounded-full border bg-white/60 text-gray-600 border-gray-300">
    {{ movie.genres[0] }}
  </span>
</div>
  ...
  <span
    ...
    :class="{
      'bg-yellow-50 border-yellow-300 text-yellow-800': genre === 'Documentaire',
      'bg-green-50 border-green-200 text-green-800': genre === 'Animation',
      'bg-purple-50 border-purple-300 text-purple-800': genre === 'Horreur',
      'bg-blue-50 border-blue-200 text-blue-800': genre === 'Drame' || genre === 'Romance',
      'bg-red-50 border-red-200 text-red-800': true
    }"
  >
    {{ genre }}
  </span>
        ...
        <div
          class="text-xs mt-1"
          :class="(movie.maxShowings - filmCounts[movie.id]) <= 0 ? 'text-red-600 font-semibold' : 'text-gray-800'"
        >
          Restants : {{ movie.maxShowings - filmCounts[movie.id] }} / {{ movie.maxShowings }}
        </div>
        <a
  ...
  <section class="flex-1">
    <h2 class="text-xl font-semibold mb-4">🗓️ Grille de programmation (Salle 1 & 2)</h2> 

      <div v-for="salle in salles" :key="salle" class="mb-8">
        <h3 class="text-lg font-bold text-blue-700 mb-2">{{ salle }}</h3>
        <div class="grid grid-cols-8 gap-1">
          <div></div>
          <div v-for="jour in jours" :key="jour" class="text-center font-bold text-orange-700">{{ jour }}</div>
          ...
          <template v-for="horaire in horaires" :key="horaire">
            ...
            >
              <div v-if="grille[salle + '-' + jour + '-' + horaire]?.title" class="w-full text-center">
...
function getSuggestedColorClass(genres) {
  ...
  if (genres.includes('Drame') || genres.includes('Romance')) return 'bg-blue-50 border-blue-200';
...
async function fetchMovies() {
  ...
  importedFilms.value = data.results
    .filter(film => film.vote_average >= minRating.value)
    .map(film => {
      const genresNames = film.genre_ids.map(id => genreMap[id] || 'Inconnu');
      const maxShowings = genresNames.includes('Documentaire') ? 2 : genresNames.includes('Animation') ? 4 : genresNames.includes('Horreur') ? 3 : 6;
...
function handleDrop(salle, jour, horaire, event) {
  ...
  if (filmCounts.value[film.id] >= film.maxShowings) {
    alert(`Le film "${film.title}" a déjà atteint son maximum de ${film.maxShowings} passages.`);
    return;
  }
...
function exportGrille() {
  const dataStr = "data:text/json;charset=utf-8," + encodeURIComponent(JSON.stringify(grille.value, null, 2));
  const dl = document.createElement('a');
  dl.setAttribute("href", dataStr);
  dl.setAttribute("download", `grille-programmation-${startDateInput.value}.json`);
...
async function fetchGenres() {
  const res = await fetch(`https://api.themoviedb.org/3/genre/movie/list?api_key=${API_KEY}&language=${LANGUAGE}`);
  const data = await res.json();
